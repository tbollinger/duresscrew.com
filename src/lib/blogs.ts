...
const getBlog = (name: string) => {
  ...
  blog.datetime = new Date(blog.date).toISOString();
  blog.datetimeParsed = new Date(blog.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric'});
  blog.tagList = blog.tags.split(',').map(tag => tag.trim());
  blog.authorUrl = `/player-profiles/${blog.author}`;
  return blog;
}
